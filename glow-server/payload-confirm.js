...
var confirmPayload = function( ack, state ) {
    return {
        phase: 'CONF',
...
confirmPayload.valid = function( ack, state, ip ) {

    const phase_valid = (typeof ack.phase !== 'undefined') && ack.phase === 'CONF';
    ...
    if ( !phase_valid && ack.phase === 'ERR' ) {
        ...
        return [ false, `The packet passed from ${ip} was not a well-formed ACK packet.`];

    } else if ( !key_valid ) {

        return [ false, `The key phrase passed from ${ip} did not match the password salt combination on this node.`];

    } else if ( !state.acknowledgements.contains( ack.key ) ) {

        return [ false, `The host ${ip} is not in the set of pending confirmations.`];
